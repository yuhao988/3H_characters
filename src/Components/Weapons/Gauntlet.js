...
export default function BrawlList(prop) {
  const { isOpen, onClose, weaponList } = prop;
  const [listOfGauntlets, setListOfGauntlets] = useState("");
  const [sortedColumn, setSortedColumn] = useState("");
  const [sortOrder, setSortOrder] = useState("asc");

  useEffect(() => {
    if (isOpen && !listOfGauntlets) {
      setListOfGauntlets(
        Object.values(weaponList).filter((weapon) => weapon.TypeID === 5)
      );
    }
  }, [isOpen, weaponList, listOfGauntlets]);
  ...
  const handleSort = (column) => {
    let sortedWeapons = [...listOfGauntlets];
    ...
    setListOfGauntlets(sortedWeapons);
  };
  ...
  return (
    <Modal isOpen={isOpen} onRequestClose={handleCloseModal}>
      ...
      <table className="stats-table">
        ...
        {listOfGauntlets ? (
          <tbody>
            {Object.values(listOfGauntlets).map((weapon, index) => (
              <tr key={index}>
                <td>{weapon.ID}</td>
                <td>{weapon.Name}</td>
                <td>{weapon.Might}</td>
