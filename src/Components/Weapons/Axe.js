...
export default function AxeList(prop) {
  const { isOpen, onClose, weaponList } = prop;
  const [listOfAxes, setListOfAxes] = useState("");
  const [sortedColumn, setSortedColumn] = useState("");
  const [sortOrder, setSortOrder] = useState("asc");

  useEffect(() => {
    if (isOpen && !listOfAxes) {
      setListOfAxes(
        Object.values(weaponList).filter((weapon) => weapon.TypeID === 3)
      );
    }
  }, [isOpen, weaponList, listOfAxes]);
  ...
  return (
    <Modal isOpen={isOpen} onRequestClose={handleCloseModal}>
      ...
      <table className="stats-table">
        ...
        {listOfAxes ? (
          <tbody>
            {Object.values(listOfAxes).map((weapon, index) => (
              <tr key={index}>
                <td>{weapon.ID}</td>
                <td>{weapon.Name}</td>
                <td>{weapon.Might}</td>
