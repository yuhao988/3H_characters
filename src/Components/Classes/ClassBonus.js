...
export default function ClassBonus() {
    ...
    const fetchData = async () => {
      try {
        ...
        for (let cl in Object.values(data)) {
          const bonus = { ID: data[cl].ID, bonus: data[cl].Bonus };
          bonusList = [...bonusList, bonus];
        }
  ...
  const handleSort = (column, rank) => {
    ...
    if (column === sortedColumn) {
      ...
      setSortOrder("asc");
      // Perform sorting bonusd on the selected column
      sortedClassList.sort((a, b) => {
        if (column === 9) {
          return totalStat(b.ID) - totalStat(a.ID);
        } else {
          return b.Bonus[column] - a.Bonus[column];
        }
      });
    }
  ...
  const displayBonus = (id, index) => {
    const cl = Object.values(classBonus).find((cls) => cls.ID === id);
    return <td>{cl.bonus[index]}</td>;
  };
            ...
            <tbody>
              {Object.values(startList).map((cl, index) => (
                <tr key={index}>
                  <td>{cl.ID}</td>
                  <td>{cl.Name}</td>
                  {intArray.map((int) => displayBonus(cl.ID, int))}
