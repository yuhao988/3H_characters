...
export default function BowList(prop) {
  const { isOpen, onClose, weaponList } = prop;
  const [listOfBows, setListOfBows] = useState("");
  const [sortedColumn, setSortedColumn] = useState("");
  const [sortOrder, setSortOrder] = useState("asc");

  useEffect(() => {
    if (isOpen && !listOfBows) {
      setListOfBows(
        Object.values(weaponList).filter((weapon) => weapon.TypeID === 4)
      );
    }
  }, [isOpen, weaponList, listOfBows]);
  ...
  return (
    <Modal isOpen={isOpen} onRequestClose={handleCloseModal}>
      ...
      <table className="stats-table">
        ...
        {listOfBows ? (
          <tbody>
            {Object.values(listOfBows).map((weapon, index) => (
              <tr key={index}>
                <td>{weapon.ID}</td>
                <td>{weapon.Name}</td>
                <td>{weapon.Might}</td>
